601,100
602,"SYS.Cube.Data.Copy"
562,"VIEW"
586,"}Dummy"
585,"}Dummy"
564,
565,"odtt]c=;jwHl<@laZ2N8dXq1F0Q2D`K5<tDK@RNq\3GNgm0oZ<`OL6QVdvHJF[K7O2QqNQyGNCF=LnQ0zmma^q2i6<?wj6L9ya`jdcLqd@VQU@Ki^]X\Ya=:yDfhcf878oj8y\YwEErfC4IycWCf`?FJIhPQ1EzIhNvSjw;w4x5\jA>rGz6VMnC48MYDE1PD`8L91iM;"
559,1
928,0
593,
594,
...
560,9
pCubeName
pSourceSubsetTypeList
pSourceSubsetElementList
...
pClearData,0
637,9
pCubeName,"pCubeName: The name of the cube in which to copy data."
pSourceSubsetTypeList,"pSourceSubsetTypeList: A comma-separated list of types for each dimension in the cube. Valid types are specified in the SYS Subset Type dimension."
pSourceSubsetElementList,"pSourceSubsetElementList: The source element for each dimension. The pSubsetTypeList property determines how each element in the list is interpreted."
pTargetSubsetTypeList,"pTargetSubsetTypeList: A comma-separated list of types for each dimension in the cube. Valid types are specified in the SYS Subset Type dimension."
pTargetSubsetElementList,"pTargetSubsetElementList: The target element for each dimension. The pSubsetTypeList property determines how each element in the list is interpreted."
pSkipZero,"pSkipZero: Enable zero-suppression on the source view."
pSkipConsolidated,"pSkipConsolidated: Ignore consolidated elements in the source view."
pSkipCalculated,"pSkipCalculated: Ignore rule calculated values in the source view."
pClearData,"pClearData: Clear the target area before copying data."
577,36
vSource_Element1
vSource_Element2
vSource_Element3
...
## Get number of dimensions in the specified cube

ExecuteProcess(sCube_Dimension_Get_List_Process_Name, 'pCubeName', pCubeName);
sCube_Dimension_List = OutputDimensionList;

ExecuteProcess(sList_Count_Process_Name, 'pItemList', sCube_Dimension_List);
nDimension_Count = OutputItemCount;

## Check that the correct number of elements is specified in the pSourceSubsetTypeList array for the specified cube

ExecuteProcess(sList_Count_Process_Name,  'pItemList', pSourceSubsetTypeList);
nElement_Type_Count = OutputItemCount;

## This process was updated to default blank SubsetElementList entries to 'A'. The validation check below is no longer required.
# IF(nElement_Type_Count <> nDimension_Count);
# 	ItemReject('The number of items in the pSubsetTypeList parameter (' | TRIM(STR(nElement_Type_Count, 15, 0)) | ') is not equal to the number of dimension in the cube (' | TRIM(STR(nDimension_Count, 15, 0)) | '). Make sure you specify an element type for each dimension in the cube.');
# 	nProcessError = 1;
# 	ProcessBreak;
# ENDIF;
...
WHILE(nCurrent_Dimension_Index <= nDimension_Count);
	...
	ExecuteProcess(sList_Read_Process_Name, 'pItemList', pSourceSubsetTypeList, 'pItemIndex', nCurrent_Dimension_Index, 'pTrimQuoteCharacters', 1);
	sCurrent_Source_Subset_Type = OutputItem;

	ExecuteProcess(sList_Read_Process_Name, 'pItemList', pSourceSubsetElementList, 'pItemIndex', nCurrent_Dimension_Index, 'pTrimQuoteCharacters', 1);
	sCurrent_Source_Element_Name = OutputItem;
	...
	sCurrent_Source_Subset_Type = IF( sCurrent_Source_Subset_Type @= '', 'A', sCurrent_Source_Subset_Type );
	sCurrent_Target_Subset_Type = IF( sCurrent_Target_Subset_Type @= '', sCurrent_Source_Subset_Type, sCurrent_Target_Subset_Type );

	sCurrent_Write_Element = '';
	sCurrent_Zero_Element = '';
	sCurrent_Selection_Type = ATTRS(sFormatSubsetName, sCurrent_Target_Subset_Type, 'Selection Type');

	IF(sCurrent_Selection_Type @= 'Single');

		sCurrent_Zero_Element = IF(sCurrent_Target_Element_Name @<> '', sCurrent_Target_Element_Name, sCurrent_Source_Element_Name);
	...
	ELSE;
		sCurrent_Zero_Element = sCurrent_Source_Element_Name;
		sCurrent_Write_Element = sCurrent_Source_Element_Name;
...
IF( pClearData = 1 );
	...
	ExecuteProcess(sCube_Data_Clear_Process_Name, 'pCubeName', pCubeName, 'pSubsetTypeList', pTargetSubsetTypeList, 'pSubsetElementList', sZero_Out_Subset_Element_List);
	# ASCIIOUTPUT('..\temp\Snapshot.txt', sZero_Out_Subset_Element_List);
ENDIF;

### Create source view and attach to process

ExecuteProcess(sCreate_Temp_Process_Name, 'pCubeName', pCubeName, 'pSubsetTypeList', pSourceSubsetTypeList, 'pSubsetElementList', pSourceSubsetElementList, 'pSkipZero', pSkipZero, 'pSkipConsolidated', pSkipConsolidated, 'pSkipCalculated', pSkipCalculated, 'pOverwriteView', 1);
...
## If a single element name was specified for an 'Single' selection type dimension, override the source element name with the specified element
...
## If an attribute name was specified for a 'Multiple' selection type dimension, override the source element name with the attribute lookup
